...
export default class WXSDK extends SDK {
    ...
    public showVideo(success: Function, quit?: Function, fail?: Function) {
        let videoId = this.getVideoAdUnitId();
        if (!videoId) {
            success();
            return;
        }
    ...
    public showBanner() {
        ...
        if (this.systemInfo.platform == "ios" && cc.visibleRect["height"] / cc.visibleRect["width"] >= 2) {
            ...
            this._bannerAd.onResize(res => {
                ...
                this._bannerAd.style.width = this.systemInfo.screenWidth + 50;
                EventManager.emit(EventType.SDKEvent.bannerResize, res.height + 20);
            });
        } else {
            this._bannerAd.onResize(res => {
                this._bannerAd.style.top = this.systemInfo.screenHeight - res.height;
                this._bannerAd.style.width = this.systemInfo.screenWidth + 50;
                EventManager.emit(EventType.SDKEvent.bannerResize, res.height);
            });
        }
